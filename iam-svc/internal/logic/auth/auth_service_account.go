...
// RefreshToken 校验旧 refresh token 后执行轮换，签发新的一对 token。
...
// Logout 使 refresh session 失效，支持单设备和全设备两种注销。
...
// GetMySession 返回当前 access token 对应的用户会话摘要。
...
// ChangePassword 在登录态下校验旧密码后更新新密码，并回收全部 refresh 会话。
...
// ResetPasswordBySms 通过短信验证码重置密码，并使历史 refresh token 全部失效。
