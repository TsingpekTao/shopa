  ...
  const login = useCallback(
    async (identifier: string, password: string) => {
      ...
      if (response.auth?.tokenPair) {
        ...
        localStorage.setItem("shopa_seller_access_token", response.auth.tokenPair.accessToken);
        if (response.auth.tokenPair.refreshToken) {
          localStorage.setItem("shopa_seller_refresh_token", response.auth.tokenPair.refreshToken);
        } else {
